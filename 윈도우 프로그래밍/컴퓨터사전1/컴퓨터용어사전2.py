...
def register():
    ...
    meaning = reg_text.get(1.0, END).strip()

    if not word:
        messagebox.showwarning("경고", "등록할 용어를 입력하세요.")
        return
    if not meaning:
        messagebox.showwarning("경고", "등록할 뜻을 입력하세요.")
        return

    if word in dic:
        answer = messagebox.askyesno("확인", f"'{word}'은(는) 이미 등록된 용어입니다.\n덮어쓰시겠습니까?")
        if not answer:
            return

    dic[word] = meaning
    reg_text.delete(1.0, END)
    output.delete(1.0, END)
    output.insert(END, f"'{word}' 용어가 등록되었습니다.")
...
# 단어 등록 영역
Label(window, text="등록할 뜻을 입력하세요:") \
.grid(row=4, column=0, sticky=W, padx=10, pady=5)

reg_text = Text(window, width=50, height=5)
reg_text.grid(row=5, column=0, sticky=W, padx=10, pady=5)

Button(window, text="단어 등록", command=register) \
